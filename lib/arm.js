...
const generateDeployScript = (outputDirectoryName) => {
    ...
    subs.getSubscriptions().forEach((subscription) => {
        subs.getResourceGroups(subscription).forEach((rg) => {
            ...
            script.push(`az group deployment create --name ${rg}-template --resource-group ${rg} --subscription ${subscription} --template-file azuredeploy.json --parameters azuredeploy.parameters.json`);
