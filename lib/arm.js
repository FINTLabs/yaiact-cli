...
const generateDeployScript = (outputDirectoryName) => {
    ...
    subs.getSubscriptions().forEach((subscription) => {
        subs.getResourceGroups(subscription).forEach((rg) => {
            ...
            script.push(`az group deployment create --name ${rg}-template --resource-group ${rg} --subscription ${subscription} --template-file ${rgPath}/azuredeploy.json`);
        });
    });
    fs.writeFileSync(path.join(outputPath, 'deploy.sh'), script.join('\n'));
