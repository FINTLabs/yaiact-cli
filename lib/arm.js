...
const generateDeployScript = (outputDirectoryName) => {
    ...
    subs.getSubscriptions().forEach((subscription) => {
        subs.getResourceGroups(subscription).forEach((rg) => {
            ...
            script.push('cwd=`dirname $0`');
            script.push(`az group deployment create --name ${rg}-template --resource-group ${rg} --subscription ${subscription} --template-file $cwd/azuredeploy.json --parameters $cwd/azuredeploy.parameters.json`);
            ...
            combinedScript.push(`${path.join(outputPath, subscription, rg,  'deploy.sh')}`);
        });
        ...
        subs.getResourceGroups(subscription).forEach((rg) => {
            ...
            subs.getResources(subscription, rg)
                .then((resources) => {
                    ...
                    validationPossible && validateArmTemplate(armFile, `${rgPath}/azuredeploy.parameters.json`, subscription, rg);
                });
        });
    });
}
